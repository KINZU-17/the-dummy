  ...
  return (
    <div className="space-y-10">
      ...
      <div className="space-y-6">
        <div className="flex flex-wrap gap-4 items-center border-b border-white/10 pb-4">
            ...
            {subTabs.map(tab => (
              <button
                ...
                className={`text-xs font-bold uppercase tracking-[0.15em] pb-2 border-b-2 transition-all ${
                  activeSubTab === tab ? 'text-white border-white/40' : 'text-white/40 border-transparent hover:text-white'
                }`}
              >
          ...
          <div className="ml-auto flex gap-3 items-center flex-wrap">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-2 bg-white text-black text-xs font-bold uppercase tracking-wider hover:bg-neutral-200 transition-colors cursor-pointer"
            >
              <Plus className="w-3.5 h-3.5" /> Add Movie
            </button>

            <select value={selectedGenre} onChange={(e) => setSelectedGenre(e.target.value)}
              className="bg-white/5 border border-white/10 text-xs px-3 py-2 text-white cursor-pointer hover:border-white/20 transition-colors outline-none">
              {genres.map(g => <option key={g} value={g} className="bg-black text-white">{g}</option>)}
            </select>

            <select value={sortBy} onChange={(e) => setSortBy(e.target.value)}
              className="bg-white/5 border border-white/10 text-xs px-3 py-2 text-white cursor-pointer hover:border-white/20 transition-colors outline-none">
              <option value="title" className="bg-black text-white">Sort: Title</option>
              <option value="year" className="bg-black text-white">Sort: Year</option>
              <option value="rating" className="bg-black text-white">Sort: Rating</option>
            </select>

            <button onClick={() => setSortOrder(sortOrder === 'asc' ? 'desc' : 'asc')}
              className="text-xs px-3 py-2 border border-white/10 text-white/60 hover:text-white hover:border-white/20 transition-all">
...
{collections.map(col => (
                 <div key={col.id} className="group bg-white/5 border border-white/10 p-4 hover:border-white/20 transition-all cursor-pointer relative">
                   <button
                   ...
                   <div className="w-full aspect-video bg-linear-to-br from-white/10 to-white/5 border border-white/10 mb-3 flex items-center justify-center">
                     <Grid className="w-8 h-8 text-white/20" />
                   </div>
                   <h4 className="text-xs font-bold uppercase tracking-wider text-white">{col.name}</h4>
                   <p className="text-[9px] text-white/40 mt-1">{col.movieCount} films</p>
                   <p className="text-[9px] text-white/30 mt-2">{col.updatedTime}</p>
                 </div>
               ))}
               <button onClick={() => setShowNewCollectionModal(true)}
                 className="bg-white/5 border border-white/10 p-4 hover:border-white/20 hover:bg-white/10 transition-all cursor-pointer flex items-center justify-center gap-2 text-xs font-bold uppercase tracking-wider text-white">
      ...
      <div>
        ...
        {filteredMovies.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-xs text-white/30 uppercase tracking-widest">Nothing here yet</p>
            <button onClick={() => setShowAddMovieModal(true)}
              className="mt-4 flex items-center gap-1.5 px-4 py-2 border border-white/10 text-xs text-white/50 hover:text-white hover:border-white/30 transition-all">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {filteredMovies.map(movie => (
              <div key={movie.id} className="group relative bg-white/5 border border-white/10 p-2 hover:border-white/20 transition-all cursor-pointer">
                <div
                  className="relative w-full aspect-3/4 bg-linear-to-b from-white/10 to-white/5 border border-white/10 mb-2 flex items-center justify-center overflow-hidden"
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                    <button onClick={(e) => { e.stopPropagation(); onPlayMovie(movie); }}
                      className="p-2.5 bg-white text-black hover:bg-neutral-200 transition-colors">
                  ...
                  <button
                    ...
                    className="absolute top-2 right-2 p-1.5 bg-black/50 hover:bg-black/70 transition-colors rounded-full"
                  >
                    <Heart className={`w-3.5 h-3.5 transition-colors ${movie.isFavorite ? 'fill-red-500 text-red-500' : 'text-white/50'}`} />
                  </button>

                  {movie.rating && (
                    <div className="absolute top-2 left-2 flex items-center gap-0.5 bg-black/60 px-1.5 py-0.5 rounded">
                ...
                <p className="text-[8px] text-white/40 mt-0.5">{movie.year} &middot; {movie.genre}</p>

                <div className="flex gap-2 mt-2">
                  <select value={movie.status || 'watchlist'} onChange={(e) => onUpdateMovieStatus(movie.id, e.target.value)}
                    onClick={(e) => e.stopPropagation()}
                    className="flex-1 bg-white/5 border border-white/10 text-[8px] px-1.5 py-1 text-white cursor-pointer outline-none hover:border-white/20">
      ...
      {history.length > 0 && (
        <div>
          ...
          <div className="space-y-3">
            {history.map(item => (
              <div key={item.id} className="flex items-center gap-4 p-3 bg-white/5 border border-white/10 hover:border-white/20 transition-all group">
                <div className="w-16 h-24 bg-linear-to-br from-white/10 to-white/5 border border-white/10 shrink-0 flex items-center justify-center">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[9px] text-white/40 mt-1">{item.watchedAt}</p>
      ...
      {showAddMovieModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#0A0A0A] border border-white/10 p-6 w-full max-w-md">
            ...
            <div className="space-y-4 mb-6">
              <input type="text" placeholder="Title" value={newMovie.title}
                ...
                className="w-full bg-white/5 border-b border-white/10 focus:border-white px-3 py-3 text-xs outline-none text-white placeholder-white/30 transition-colors" autoFocus />
              <div className="grid grid-cols-2 gap-3">
                <input type="number" placeholder="Year" value={newMovie.year}
                  onChange={(e) => setNewMovie({ ...newMovie, year: e.target.value })}
                  className="bg-white/5 border-b border-white/10 focus:border-white px-3 py-3 text-xs outline-none text-white placeholder-white/30 transition-colors" />
              ...
              <div className="grid grid-cols-2 gap-3">
                <select value={newMovie.genre} onChange={(e) => setNewMovie({ ...newMovie, genre: e.target.value })}
                  className="bg-white/5 border-b border-white/10 focus:border-white px-3 py-3 text-xs text-white cursor-pointer outline-none transition-colors">
                  {genres.filter(g => g !== 'All Genres').map(g => <option key={g} value={g} className="bg-black text-white">{g}</option>)}
                </select>
            ...
            <div className="flex gap-3">
              <button onClick={() => { setShowAddMovieModal(false); setNewMovie({ title: '', year: '', genre: 'Drama', duration: '', rating: '' }); }}
                className="flex-1 px-4 py-2 border border-white/20 text-xs font-bold uppercase tracking-wider text-white hover:bg-white/5 transition-colors cursor-pointer">
                Cancel
              </button>
              <button onClick={handleAddMovie} disabled={!newMovie.title.trim()}
                className="flex-1 px-4 py-2 bg-white text-black text-xs font-bold uppercase tracking-wider hover:bg-neutral-200 transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed">
                Add to Library
              </button>
            </div>
          </div>
          ...
          <div className="bg-[#0A0A0A] border border-white/10 p-6 w-full max-w-md">
            ...
            <div className="mb-6">
              <input type="text" value={newCollectionName} onChange={(e) => setNewCollectionName(e.target.value)}
                ...
                className="w-full bg-white/5 border-b border-white/10 focus:border-white px-3 py-3 text-xs outline-none text-white placeholder-white/30 transition-colors" />
            </div>
            <div className="max-h-64 overflow-y-auto mb-6 space-y-2">
              {movies.map(movie => (
                <label key={movie.id} className="flex items-center gap-3 p-2 cursor-pointer hover:bg-white/5 transition-colors">
